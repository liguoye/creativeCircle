<template>
  <div class="goodsManagement">
        ...
        <div class="btnGroup">
          <el-row>
            ...
            >
              <span>掌柜号: </span>
              <el-select
                v-model="formData.manager.value"
                placeholder="请选择"
              >
                <el-option
                  v-for="(item,index) in formData.manager.options"
                  :key="index"
    ...
    <add-dialog
      ...
      :dialog-table-visible="addDialogShow"
      :passdata='formData.manager.options'
      @submitSuccess="getGoodsData"
    ></add-dialog>
    <edit-dialog
      @dialogClose="editDialogClose"
      :table-row-data="tableData.data[tableData.rowIndex]"
      :dialog-table-visible="editDialogShow"
      :check-detail="tableData.check"
      :passdata='formData.manager.options'
      @editConfirm="editConfirm"
    ></edit-dialog>
    <task-dialog
      :dialog-table-visible="taskDialogShow"
      @dialogClose="taskDialogClose"
    ></task-dialog>
    <buy-dialog
      :dialog-table-visible="buyDialogShow"
      @dialogClose="buyDialogClose"
    ></buy-dialog>
  </div>
</template>
<script>
...
export default {
    ...
    return {
      ...
      tableData: {
        ...
        columns: [
          ...
          { name: "商品ID", code: "tb_goodsid", width: "" },
  ...
  methods: {
    ...
    deleteGoods() {
      this.$confirm(
        "将删除商品:" +
          this.tableData.data[this.tableData.rowIndex]["title"] +
          ", 是否继续 ? ",
        "提示",
        {
